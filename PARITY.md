...
范围：只读对比原始 TypeScript 源码（`/home/bellman/Workspace/claude-code/src/`）与 Rust 移植版（`rust/crates/`）。

方法：仅对比功能面、注册表、入口点和运行时管线。未复制任何 TypeScript 源码。
...
- Hook config can be inspected via Rust config reporting in `rust/crates/commands/src/lib.rs` and `rust/crates/yunxi-cli/src/main.rs`.
...
- Rust supports `/memory` and `/init` via `rust/crates/commands/src/lib.rs` and `rust/crates/yunxi-cli/src/main.rs`.
...
- Main CLI/repl/prompt handling lives in `rust/crates/yunxi-cli/src/main.rs`.
...
- Stream/tool event translation in `rust/crates/yunxi-cli/src/main.rs`.
...
- **Prompt mode tools enabled**
  - `rust/crates/yunxi-cli/src/main.rs` now constructs prompt mode with `LiveCli::new(model, true, ...)`.
- **Default permission mode = DangerFullAccess**
  - Runtime default now resolves to `DangerFullAccess` in `rust/crates/yunxi-cli/src/main.rs`.
  - Clap default also uses `DangerFullAccess` in `rust/crates/yunxi-cli/src/args.rs`.
  - Init template writes `dontAsk` in `rust/crates/yunxi-cli/src/init.rs`.
- **Streaming `{}` tool-input prefix bug**
  - `rust/crates/yunxi-cli/src/main.rs` now strips the initial empty object only for streaming tool input, while preserving legitimate `{}` in non-stream responses.
