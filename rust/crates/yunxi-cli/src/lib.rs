...
pub mod oauth_flow;
pub use cli_action::normalize_startup_model;
mod permission_ui;
mod render;
mod routing;
...
/// 将合并配置中的 `env` 段注入进程环境（不覆盖已有非空变量）。
fn apply_merged_config_env() {
    let cwd = session_mgr::workspace_root()
        .or_else(|_| env::current_dir())
        .unwrap_or_else(|_| PathBuf::from("."));
    let Ok(cfg) = ConfigLoader::default_for(cwd).load() else {
        return;
    };
    let Some(env_obj) = cfg.get("env").and_then(|v| v.as_object()) else {
        return;
    };
...
mod tests {
 ...
 M src/main.rs",
    ...
    fn status_context_reads_real_workspace_metadata() {
        ...
        assert!(context.discovered_config_files >= 8);
