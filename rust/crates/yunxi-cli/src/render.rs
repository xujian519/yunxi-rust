...
impl ThinkingIndicator {
    ...
    pub fn tick(&mut self, theme: &ColorTheme, out: &mut impl Write) -> io::Result<()> {
        ...
        let label = format!("思考中... {:.1}s", elapsed.as_secs_f64());
    ...
    pub fn finish(
        ...
        let label = if tokens > 0 {
            format!("完成 ({:.1}s, {} tokens)", elapsed.as_secs_f64(), tokens)
        } else {
            format!("完成 ({:.1}s)", elapsed.as_secs_f64())
        };
...
mod tests {
    ...
    fn renders_links_as_colored_markdown_labels() {
        let terminal_renderer = TerminalRenderer::new();
        let markdown_output =
            terminal_renderer.render_markdown("See [Yunxi](https://example.com/docs) now.");
        let plain_text = strip_ansi(&markdown_output);

        assert!(plain_text.contains("[Yunxi](https://example.com/docs)"));
        assert!(markdown_output.contains('\u{1b}'));
    }
    ...
    #[test]
    fn thinking_indicator_tick_shows_elapsed_time() {
        let mut indicator = super::ThinkingIndicator::new();
        let mut out = Vec::new();
        let theme = *TerminalRenderer::new().color_theme();
        indicator.tick(&theme, &mut out).expect("tick succeeds");
        let output = String::from_utf8_lossy(&out);
        assert!(output.contains("思考中..."));
    }

    #[test]
    fn progress_bar_renders_correctly() {
        let bar = super::ProgressBar::new(10);
        assert_eq!(bar.render(), "[░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░] 0/10");

        let mut bar = super::ProgressBar::new(10);
        bar.advance();
        bar.advance();
        bar.advance();
    ...
    fn progress_bar_zero_total_is_empty() {
        let bar = super::ProgressBar::new(0);
