...
impl TuiApp {
    ...
    fn refresh_slash_completion(&mut self) {
        ...
        self.slash_completion = SlashCompletion::refresh(content, true);
    }
    ...
    /// 仅输入 `/` 或尚未写完命令名时，回车先应用补全项再提交。
    fn should_apply_slash_completion_on_enter(&self) -> bool {
        let t = self.input.content().trim();
        t == "/" || (t.starts_with('/') && !t[1..].contains(char::is_whitespace))
    }
    ...
    fn apply_tab_completion(&mut self) -> bool {
        ...
        if SlashCompletion::refresh(self.input.content(), true).is_some() {
            self.refresh_slash_completion();
            return true;
        }
        false
    ...
    pub(crate) fn handle_key(&mut self, key: &KeyEvent) -> Option<TuiAction> {
        ...
        match key {
            ...
            KeyEvent::Enter if !self.input.is_empty() && !self.has_blocking_modal() => {
                if self.should_apply_slash_completion_on_enter() {
    ...
    fn paint_modal_overlays(&self, frame: &mut Frame, width: u16, height: u16) {
        ModalOverlays {
            width,
            height,
    ...
    pub(crate) fn render_with_cursor(&self, width: u16, height: u16) -> String {
        let mut rendered = self.render_general(width, height);
    ...
    fn input_cursor_pos(&self, width: u16, height: u16) -> Option<(u16, u16)> {
        ...
        let input_bar = self.general_layout(width, height).input_bar;
    ...
    fn render_general(&self, width: u16, height: u16) -> String {
        ...
        self.paint_modal_overlays(&mut frame, width, height);
...
mod tests {
    ...
    fn test_app() -> TuiApp {
        TuiApp::new("deepseek-v4-pro".to_string(), "0.1.0".to_string())
    }
