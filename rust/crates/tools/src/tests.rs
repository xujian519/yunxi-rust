...
use super::{
    ...
    final_assistant_text, mvp_tool_specs, persist_agent_terminal_state, AgentInput, AgentJob,
...
fn skill_loads_local_skill_prompt() {
    ...
    let output: serde_json::Value = serde_json::from_str(&result).expect("valid json");
    assert_eq!(output["skill"], "patent-retrieval");
    assert!(output["path"]
        .as_str()
        .expect("path")
        .ends_with("/patent-retrieval/SKILL.md"));
    ...
    assert_eq!(dollar_output["skill"], "$patent-retrieval");
...
#[test]
fn skill_loads_builtin_patent_skill_from_assets() {
    // 验证能从项目 assets/skills/ 加载专利 skill
    let result = execute_tool(
        "Skill",
        &json!({
            "skill": "cap-retrieval"
    ...
    .expect("Skill should load builtin patent skill from assets/skills/");
...
#[test]
fn skill_loads_patent_retrieval_skill_with_yaml_frontmatter() {
    let result = execute_tool(
        "Skill",
        &json!({
            "skill": "patent-retrieval"
    ...
    .expect("Skill should load patent-retrieval skill");
    ...
    assert!(output["description"]
        .as_str()
        .expect("description")
        .contains("search patents"));
}

#[test]
fn knowledge_graph_query_searches_guideline_and_legal() {
    let graph_dir = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../../assets/knowledge_graph"
    );

    // 强制使用 JSON 回退（通过指定不存在的 sqlite_path）
    let result = execute_tool(
        "KnowledgeGraphQuery",
        &json!({
            "query": "新颖性",
            ...
            "limit": 5,
            "graph_dir": graph_dir,
            "sqlite_path": "/dev/null/nonexistent_db"
        }),
    )
    .expect("KnowledgeGraphQuery should succeed");

    let output: serde_json::Value = serde_json::from_str(&result).expect("valid json");
    assert!(
        output["total_found"].as_u64().unwrap_or(0) > 0,
        "should find novelty-related nodes, got: {result}"
    );
    assert!(!output["results"].as_array().unwrap().is_empty());

    // 查询法律知识图谱
    let legal_result = execute_tool(
        "KnowledgeGraphQuery",
        &json!({
            "query": "专利法",
            "source": "legal",
    ...
    .expect("KnowledgeGraphQuery legal should succeed");

    let legal_output: serde_json::Value = serde_json::from_str(&legal_result).expect("valid json");
    assert!(
        legal_output["total_found"].as_u64().unwrap_or(0) > 0,
        "should find patent law entities"
    );

    // 联合查询
    let all_result = execute_tool(
        "KnowledgeGraphQuery",
        &json!({
            "query": "专利",
            "source": "all",
    ...
    .expect("KnowledgeGraphQuery all should succeed");

    let all_output: serde_json::Value = serde_json::from_str(&all_result).expect("valid json");
    assert!(
        all_output["total_found"].as_u64().unwrap_or(0) > 0,
        "should find results from both sources"
    );
    assert!(all_output["stats"]["guideline_nodes"].as_u64().unwrap_or(0) > 0);
}

#[test]
fn knowledge_graph_query_uses_sqlite_when_available() {
    // 使用项目内的 SQLite 数据库路径
    let db_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
    ...
    if !std::path::Path::new(db_path).exists() {
        eprintln!("Skipping SQLite test: DB not found at {db_path}");
        return;
    }
    ...
    .expect("KnowledgeGraphQuery SQLite should succeed");

    let output: serde_json::Value = serde_json::from_str(&result).expect("valid json");
    assert!(
        output["total_found"].as_u64().unwrap_or(0) > 0,
        "SQLite should find results for 新颖性"
    );

    // 验证 stats 包含 SQLite 统计
    let stats = output["stats"].as_object().expect("stats");
    assert!(
        stats.contains_key("sqlite_nodes"),
        "should have sqlite_nodes stat"
    );
    assert!(
        stats["sqlite_nodes"].as_i64().unwrap_or(0) > 0,
        "sqlite_nodes should be > 0"
    );
    assert!(
        stats["sqlite_edges"].as_i64().unwrap_or(0) > 0,
        "sqlite_edges should be > 0"
    );

    // 验证结果来源标记
    let first = output["results"].as_array().unwrap().first().unwrap();
    assert_eq!(first["source_type"], "sqlite");

    // 按类型筛选
    let type_result = execute_tool(
        "KnowledgeGraphQuery",
        &json!({
            "query": "专利",
    ...
    .expect("KnowledgeGraphQuery with node_type should succeed");

    let type_output: serde_json::Value = serde_json::from_str(&type_result).expect("valid json");
    for item in type_output["results"].as_array().unwrap() {
        assert_eq!(item["item_type"], "GuidelineRule");
    }
}
