...
pub fn execute_tool(name: &str, input: &Value) -> Result<String, String> {
    match name {
        ...
        "WebFetch" => from_value::<web::WebFetchInput>(input).and_then(run_web_fetch),
        "WebSearch" => from_value::<web::WebSearchInput>(input).and_then(run_web_search),
        "TodoWrite" => from_value::<todo::TodoWriteInput>(input).and_then(run_todo_write),
        "Skill" => from_value::<skill::SkillInput>(input).and_then(run_skill),
        "Agent" => from_value::<AgentInput>(input).and_then(run_agent),
        "ToolSearch" => from_value::<ToolSearchInput>(input).and_then(run_tool_search),
        "NotebookEdit" => {
            from_value::<notebook::NotebookEditInput>(input).and_then(run_notebook_edit)
        }
        "Sleep" => from_value::<brief::SleepInput>(input).and_then(run_sleep),
        "SendUserMessage" | "Brief" => from_value::<brief::BriefInput>(input).and_then(run_brief),
        "Config" => from_value::<config_tool::ConfigInput>(input).and_then(run_config),
        "StructuredOutput" => {
            from_value::<config_tool::StructuredOutputInput>(input).and_then(run_structured_output)
        }
...
#[allow(clippy::needless_pass_by_value)]
fn run_web_fetch(input: web::WebFetchInput) -> Result<String, String> {
    to_pretty_json(web::execute_web_fetch(&input)?)
}

#[allow(clippy::needless_pass_by_value)]
fn run_web_search(input: web::WebSearchInput) -> Result<String, String> {
    to_pretty_json(web::execute_web_search(&input)?)
}

fn run_todo_write(input: todo::TodoWriteInput) -> Result<String, String> {
    to_pretty_json(todo::execute_todo_write(input)?)
}

fn run_skill(input: skill::SkillInput) -> Result<String, String> {
    to_pretty_json(skill::execute_skill(input)?)
}

fn run_agent(input: AgentInput) -> Result<String, String> {
    to_pretty_json(agent::execute_agent(input)?)
}
...
fn run_notebook_edit(input: notebook::NotebookEditInput) -> Result<String, String> {
    to_pretty_json(notebook::execute_notebook_edit(input)?)
}

fn run_sleep(input: brief::SleepInput) -> Result<String, String> {
    to_pretty_json(brief::execute_sleep(input))
}

fn run_brief(input: brief::BriefInput) -> Result<String, String> {
    to_pretty_json(brief::execute_brief(input)?)
}

fn run_config(input: config_tool::ConfigInput) -> Result<String, String> {
    to_pretty_json(config_tool::execute_config(input)?)
}

fn run_structured_output(input: config_tool::StructuredOutputInput) -> Result<String, String> {
    to_pretty_json(config_tool::execute_structured_output(input))
}

fn run_repl(input: repl::ReplInput) -> Result<String, String> {
...
// --- Input structs kept in lib.rs for dispatcher use ---
  ...
  "nbformat_minor": 5
    ...
    fn bash_tool_reports_success_exit_failure_timeout_and_background() {
        ...
        let failure = execute_tool(
            "bash",
            &json!({ "command": "printf 'oops' >&2; exit 7", "dangerously_disable_sandbox": true }),
        )
        ...
        let timeout = execute_tool(
            "bash",
            &json!({ "command": "sleep 1", "timeout": 10, "dangerously_disable_sandbox": true }),
        )
        ...
        let background = execute_tool(
            "bash",
            &json!({ "command": "sleep 1", "run_in_background": true, "dangerously_disable_sandbox": true }),
        )
